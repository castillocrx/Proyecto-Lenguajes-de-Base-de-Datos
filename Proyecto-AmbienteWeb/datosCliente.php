...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nombre == "") {
        $errores[] = "No se digito el nombre del cliente";
    } else {
        $nombreOK = true;
    }
    if ($correo == "") {
        $errores[] = "No se digito el correo del cliente";
    } else {
        $correoOK = true;
    }
    if ($direccion == "") {
        $errores[] = "No se digito direccion del cliente";
    } else {
        $direccionOK = true;
    }
    if ($telefono == "") {
        $errores[] = "No se digito el telefono del cliente";
    } else {
        $telefonoOK = true;
    }
    if ($contrasena == "") {
        $errores[] = "No se digito contraseña del cliente";
    } else {
        $contrasenaOK = true;
    }
    ...
    if ($nombreOK && $correoOK && $direccionOK && $telefonoOK && $contrasenaOK) {
        ...
        if (IngresoCliente($nombre, $correo, $direccion, $telefono, $contrasena)) {
...
<main class="contenedor-añadirP">
    <h1 class="pos-h1">Registrar la información del cliente</h1>
